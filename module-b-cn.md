...
- 種子員工密碼（所有帳號）：`password123`。種子資料中的 `password_hash` 值為 bcrypt。請以 bcrypt 驗證。請勿改用其他雜湊演算法取代它們。

若使用 Node.js，可使用 `bcryptjs` 函式庫來驗證與儲存密碼：
...
使用所提供的資料庫傾印。請勿重新命名評分所需的資料表或欄位。請保留種子資料中的 bcrypt `password_hash` 值。
...
- 員工以電子郵件與密碼對 `staff_users` 登入，並使用伺服器端 session。請以 bcrypt 驗證密碼。
...
- 新增使用者時密碼為必填，並須以 bcrypt 儲存。編輯時若密碼留空，則保留既有雜湊；新密碼也必須以 bcrypt 儲存。
